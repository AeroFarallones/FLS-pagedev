...
namespace Modules\AeroFarallonesTheme\Services;
...
class Fls_StatServices
{
    ...
    public function PersonalStats($user_id, $period, $type)
    {
        ...
        if ($period === 'currenty' || $period === 'lasty' || $period === 'prevy') { // Years
            ...
            $personal['period_text'] = __('Fls::dates.' . $b_date->format('m'));
        } elseif (is_numeric($period)) { // Days
            $s_date = $b_date->copy()->startOfDay()->subdays($period);
            $e_date = $now->endOfDay();
            $personal['period_text'] = __('Fls::widgets.lastndays', ['period' => $period]);
    ...
    public function BasicStats($airline_id = null)
    {
        ...
        if (empty($airline_id)) {
            $stats[__('Fls::common.airlines')] = DB::table('airlines')->where('active', 1)->count();
        }

        $stats[__('Fls::common.pilots')] = DB::table('users')->where($where)->count();
        $stats[__('Fls::common.subfleets')] = count($subfleets_array);
        $stats[__('Fls::common.aircraft')] = DB::table('aircraft')->whereIn('subfleet_id', $subfleets_array)->count();
        $stats[__('Fls::common.flights')] = DB::table('flights')->where($where)->count();
    ...
    public function PirepStats($airline_id = null, $aircraft_id = null)
    {
        ...
        $stats[__('Fls::widgets.pireps_ack')] = DB::table('pireps')->where($where)->count();

        // Return null if pirep count is zero, no need to work for the rest
        if ($stats[__('Fls::widgets.pireps_ack')] === 0) {
            return null;
        }
        ...
        if (setting('pireps.delete_rejected_hours') == 0 && $level > 10) {
            if (empty($airline_id)) {
                $stats[__('Fls::widgets.pireps_rej')] = DB::table('pireps')->where('state', PirepState::REJECTED)->count();
            } else {
                $stats[__('Fls::widgets.pireps_rej')] = DB::table('pireps')->where(['airline_id' => $airline_id, 'state' => PirepState::REJECTED])->count();
            }
        }
        */
        ...
        if ($pax_amount > 0) {
            $stats[__('Fls::widgets.pireps_pax')] = number_format($pax_amount);
            $stats[__('Fls::widgets.avg_pax')] = number_format($pax_avg);
        }

        if ($cgo_amount > 0) {
            $stats[__('Fls::widgets.pireps_cgo')] = number_format($cgo_amount) . ' ' . $unit_weight;
            $stats[__('Fls::widgets.avg_cgo')] = number_format($cgo_avg) . ' ' . $unit_weight;
        }
        ...
        $stats[__('Fls::widgets.ttime')] = DB_ConvertMinutes($total_time, '%2dh %2dm');
        if ($level > 10) {
            $stats[__('Fls::widgets.atime')] = DB_ConvertMinutes($average_time, '%2dh %2dm');
        }

        $stats[__('Fls::widgets.tfuel')] = number_format($total_fuel) . ' ' . $unit_fuel;
        if ($level > 10) {
            $stats[__('Fls::widgets.afuel')] = number_format($average_fuel) . ' ' . $unit_fuel;
        }

        if ($total_fuel > 0 && $total_time > 0) {
            $average_fuel_hour = ($total_fuel / $total_time) * 60;
            $stats[__('Fls::widgets.hfuel')] = number_format($average_fuel_hour) . ' ' . $unit_fuel;
        }

        $stats[__('Fls::widgets.tdist')] = number_format($total_dist) . ' ' . $unit_distance;
        if ($level > 10) {
            $stats[__('Fls::widgets.adist')] = number_format($average_dist) . ' ' . $unit_distance;
        }

        if ($total_dist > 0 && $total_time > 0 && $level > 10) {
            $average_dist_hour = ($total_dist / $total_time) * 60;
            $stats[__('Fls::widgets.hdist')] = number_format($average_dist_hour) . ' ' . $unit_distance;
        }

        $where['source'] = PirepSource::ACARS;

        $average_lrate = DB::table('pireps')->where($where)->avg('landing_rate');
        $stats[__('Fls::widgets.alrate')] = number_format(abs($average_lrate)) . ' ft/min';

        if ($level > 10) {
            $average_score = DB::table('pireps')->where($where)->avg('score');
            $stats[__('Fls::widgets.ascore')] = number_format($average_score);
        }
    ...
    public function AirlineFinance($journal_id)
    {
        ...
        $finance[__('Fls::common.income')] = money($income, $currency);
        $finance[__('Fls::common.expense')] = money($expense, $currency);
        $finance[__('Fls::common.balance')] = '<span style="color: ' . $color . ';"><b>' . money($balance, $currency) . '</b></span>';
