...
class Fls_AdminController extends Controller
{
    public function index()
    {
        // Get settings (of Fls Basic)
        $settings = DB::table('Fls_settings')->where('key', 'LIKE', 'FlsModule.%')->get();
        ...
        return view('FlsModule::admin.index', [
            'awards'   => $awards,
            'settings' => $settings,
            'users'    => $users,
        ]);
    ...
    public function manual_payment()
    {
        ...
        if (filled($user) && $user->airline->journal->balance > $amount) {
            ...
            Log::info('Fls Basic | Bonus Payment of ' . $amount . ' to ' . $user->name_private . ' completed by ' . Auth::user()->name_private);
        } else {
            flash()->error('Airline balance is NOT enough to complete wire transfer !');
            Log::info('Fls Basic | Bonus Payment Result: Rejected, Not Enough Funds');
        }
    ...
    public function settings_update()
    {
        ...
        foreach ($formdata as $id => $value) {
            ...
            $setting = DB::table('Fls_settings')->where('id', $id)->first();

            if (!$setting) {
                continue;
            }

            Log::debug('Fls Basic, ' . $setting->group . ' setting for ' . $setting->name . ' changed to ' . $value);
            DB::table('Fls_settings')->where(['id' => $setting->id])->update(['value' => $value]);
        }

        flash()->success($section . ' settings saved.');
        return back(); // return redirect(route('FlsModule.admin'));
    }
    ...
    public function health_check()
    {
        ...
        return view('FlsModule::admin.health_check', [
