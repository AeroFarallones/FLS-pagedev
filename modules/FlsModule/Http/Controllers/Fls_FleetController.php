...
use Modules\FlsSpecial\Models\DS_Maintenance;

class Fls_FleetController extends Controller
{
    // Fleet
    public function index()
    {
        ...
        return view('FlsModule::fleet.index', [
            'aircraft' => $aircraft,
            'units'    => $this->GetUnits(),
        ]);
    }
    ...
    public function subfleet($subfleet_type)
    {
        ...
        if (!$subfleet) {
            ...
            return redirect(route('FlsModule.fleet'));
        }
        ...
        // Specifications
        $specs = Fls_GetSpecs_SF($subfleet, true);
        ...
        return view('FlsModule::fleet.subfleet', [
            'aircraft' => $aircraft,
    ...
    public function aircraft($ac_reg)
    {
        ...
        // Specifications
        $specs = Fls_GetSpecs($aircraft, true);

        // Maintenance Status
        $maint = Fls_CheckModule('FlsSpecial') ? DS_Maintenance::where('aircraft_id', $aircraft->id)->first() : null;
        ...
        return view('FlsModule::fleet.aircraft', [
            'aircraft' => $aircraft,
            'files'    => filled($files) ? $files : null,
            'image'    => $image,
