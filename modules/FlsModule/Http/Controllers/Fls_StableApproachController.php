...
class Fls_StableApproachController extends Controller
{
    public function index()
    {
        $user = Auth::user();
        $sap_reports = Fls_StableApproach::with('user', 'pirep')->where('user_id', $user->id)->orderBy('created_at', 'desc')->paginate(12);

        return view('FlsModule::sap.index', [
    ...
    {
        // Check Settings
        $status = (Fls_Setting('FlsModule.stable_app_control', false) === true) ? null : 'Plugin support disabled';
        ...
        if (!isset($status)) {
            // Check Duplicate
            $duplicate = Fls_StableApproach::where('sap_analysisID', $report->analysis->id)->count();
        ...
        if (!isset($status)) {
            // Check User
            $where = ['name' => Fls_Setting('FlsModule.stable_app_field', 'Stable Approach ID'), 'active' => true];
        ...
        if (!isset($status)) {
            // Save the report
            Fls_StableApproach::updateOrCreate([
            ...
            Log::debug('Fls Basic, Stable Approach Report RECEIVED (A: ' . $report->analysis->id . ', P: ' . $pirep_id . ', U: ' . $user_id . ')');
        } else {
            // Add reason to the response
            $response['received'] = 'rejected';
            $response['reason'] = $status;
            Log::debug('Fls Basic, Stable Approach Report REJECTED (' . $status . ')');
        }
    ...
    {
        $report = Fls_StableApproach::where(['id' => $request->report_id, 'is_stable' => 0])->first();
