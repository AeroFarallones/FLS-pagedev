...
class Fls_WidgetController extends Controller
{
    // Aircraft Transfer
    public function transferac(Request $request)
    {
        ...
        if (!$selected_ac) {
            flash()->error(__('FlsModule::widgets.ta_err_ac'));
        ...
        if ($price === 'free') {
            ...
            $aircraft->save();
            flash()->success(__('FlsModule::widgets.ta_ok_free', ['registration' => $aircraft->registration]));
            Log::info('Fls Basic | Free Aircraft Transfer > ' . $aircraft->registration . ' moved to ' . $user_location . ' by ' . $user->name_private);
        ...
        if ($price === 'auto') {
            ...
            Log::debug('Fls Basic | Aircraft Transfer > Transfer Distance: ' . $transfer_distance);
            Log::debug('Fls Basic | Aircraft Transfer > Fuel Price: ' . $fuel_price);
            Log::debug('Fls Basic | Aircraft Transfer > Fuel Burn: ' . $avrg_fuelburn);
            Log::debug('Fls Basic | Aircraft Transfer > Fuel Used: ' . $aprx_fuelburn);
            Log::debug('Fls Basic | Aircraft Transfer > Fuel Cost: ' . $fuel_cost);
            Log::debug('Fls Basic | Aircraft Transfer > GH Cost: ' . $gh_cost);
            Log::debug('Fls Basic | Aircraft Transfer > Calculated Cost: ' . $transfer_cost);
        }

        // Transfer price is fixed (Define cost, continue)
        if (is_numeric($price)) {
            $transfer_cost = Money::createFromAmount($price);
            Log::debug('Fls Basic | Aircraft Transfer > Fixed Cost: ' . $transfer_cost);
        }
        ...
        if ($transfer_cost > $user->journal->balance) {
            flash()->error(__('FlsModule::widgets.ta_err_funds', ['price' => $transfer_cost]));
        ...
        // Flash Message
        if ($price === 'auto') {
            flash()->success(__('FlsModule::widgets.ta_ok_auto', ['registration' => $aircraft->registration, 'price' => $transfer_cost, 'distance' => $transfer_distance]));
        } else {
            flash()->success(__('FlsModule::widgets.ta_ok_fixed', ['registration' => $aircraft->registration, 'price' => $transfer_cost]));
        }

        // Move Asset (complete)
        $aircraft->airport_id = $user_location;
        $aircraft->save();
        Log::info('Fls Basic | Aircraft Transfer > ' . $aircraft->registration . ' moved to ' . $user_location . ' by ' . $user->name_private . '. Price: ' . $transfer_cost);
    ...
    public function jumpseat(Request $request)
    {
        ...
        if (!$new_location || $new_location == $user_location) {
            flash()->error(__('FlsModule::widgets.js_err_dest'));
        ...
        if ($price === 'free') {
            ...
            $user->save();
            flash()->success(__('FlsModule::widgets.js_ok_free', ['location' => $new_location]));
        ...
        // Check User Balance (abort or continue)
        if ($transfer_cost > $user->journal->balance) {
            flash()->error(__('FlsModule::widgets.js_err_funds', ['price' => $transfer_cost]));
        ...
        // Flash Message
        if ($price === 'auto') {
            flash()->success(__('FlsModule::widgets.js_ok_auto', ['location' => $new_location, 'price' => $transfer_cost, 'distance' => $transfer_distance]));
        } else {
            flash()->success(__('FlsModule::widgets.js_ok_fixed', ['location' => $new_location, 'price' => $transfer_cost]));
        }
