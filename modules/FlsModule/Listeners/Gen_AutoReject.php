...
class Gen_AutoReject
{
    ...
    {
        $auto_reject = Fls_Setting('FlsModule.autoreject', false);

        $margin_score = Fls_Setting('FlsModule.ar_marginscore', 0);
        $margin_lrate = Fls_Setting('FlsModule.ar_marginlrate', 0);
        $margin_ftime = Fls_Setting('FlsModule.ar_marginftime', 0);
        $margin_fburn = Fls_Setting('FlsModule.ar_marginfburn', 0);
        $margin_thrdist = Fls_Setting('FlsModule.ar_marginthrdist', 0);
        $margin_gforce = Fls_Setting('FlsModule.ar_margingforce', 0);
        $margin_pause = Fls_Setting('FlsModule.ar_marginpause', 0);
        $margin_tdiff = Fls_Setting('FlsModule.ar_margintdiff', 0);
        $margin_presence = Fls_Setting('FlsModule.networkcheck_margin', 80);
        $reject_presence = Fls_Setting('FlsModule.ar_presence', false);
        $reject_callsign = Fls_Setting('FlsModule.ar_callsign', false);
        ...
        if ($acars_pirep && $reject_presence && isset($network_presence) && $network_presence < $margin_presence) {
            ...
            $pirep_state = PirepState::REJECTED;
            Log::info('Fls Basic | Pirep:' . $pirep->id . ' Rejected automatically by Presence. Check Result:' . $network_presence . '% Requirement:' . $margin_presence . '%');
        }
        ...
        if ($acars_pirep && $reject_callsign && isset($network_callsign) && isset($network_presence) && $network_presence > 0 && $network_callsign < $margin_presence) {
            ...
            $pirep_state = PirepState::REJECTED;
            Log::info('Fls Basic | Pirep:' . $pirep->id . ' Rejected automatically by Callsign. Check Result:' . $network_callsign . '% Requirement:' . $margin_presence . '%');
        }
