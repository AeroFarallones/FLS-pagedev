...
class Pirep_Prefiled
{
    ...
    {
        if (Fls_Setting('FlsModule.acstate_control') && $event->pirep->aircraft && $event->pirep->source === 0) {
            $aircraft = $event->pirep->aircraft;
            $aircraft->state = AircraftState::IN_USE;
            $aircraft->save();
            Log::info('Fls Basic | Pirep:' . $event->pirep->id . ' PRE-FILED, Changed STATE of ' . $aircraft->registration . ' to IN USE');
        }
    }
}
