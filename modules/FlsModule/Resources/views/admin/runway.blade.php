@extends('admin.app')
@section('title', 'Fls Runway Management')
...
<div class="card border-blue-bottom" style="margin-left:5px; margin-right:5px; margin-bottom:5px;">
  <div class="content">
    ...
    <p>
      This section is designed for quick management of a runway or some runways of an airport, it is not designed to
      insert all runways at once or for mass updating.
  ...
  <div class="card border-blue-bottom" style="padding:10px;">
    <div class="row" style="margin-bottom: 10px;">
      <div class="col-sm-4">
        ...
        <select id="icao_selection" name="airport_id" class="form-control select2" style="width: 100%;"
          onchange="Check_Airport_Selection()">
          <option value="0">Please select an airport...</option>
          @foreach($airports_r as $airport)
          <option value="{{ $airport->id }}" @if($selected_a && $selected_a==$airport->id) selected @endif>{{
            $airport->id.' : '.$airport->name }}</option>
          @endforeach
        </select>
      </div>
      <div class="col-sm-3 text-left align-middle"><br>
        <a id="load_link" style="visibility: hidden" href="{{ route('FlsModule.runway') }}"
          class="btn btn-primary pl-1 mb-1">Load Runways of Selected Airport</a>
      </div>
      <div class="col-sm-1">
    ...
    @include('FlsModule::admin.runway_form')
    @if(!$loop->last)
    <hr> @endif
    @endforeach
    @endif
    {{-- New Runway Entry --}}
    <hr>
    @include('FlsModule::admin.runway_form', ['runway' => null])
  </div>
</div>
...
<script type="text/javascript">
  // Simple selection with dropdown change
    ...
    const $main_link = String("{{ route('FlsModule.runway') }}");
