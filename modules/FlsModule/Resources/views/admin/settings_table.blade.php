...
<div style="margin-bottom: 5px;">
  {{ Form::open(['route' => 'FlsModule.settings_update', 'method' => 'post']) }}
  <table class="table table-striped text-left" style="margin-bottom: 2px;">
    ...
    <tr>
      ...
      <td>
        ...
        <input class="form-control" type="checkbox" name="{{ $st->id }}" value="true" @if($st->value === 'true') checked
        @endif>
        @elseif($st->field_type === 'select')
        @php $values = explode(',', $st->options); @endphp
        <select class="form-control" name="{{ $st->id }}">
          @foreach($values as $value)
          <option value="{{ $value }}" @if($st->value === $value || !filled($st->value) && $st->default === $value)
            selected @endif>{{ $value }}</option>
          @endforeach
        </select>
        @else
        <input class="form-control" @if($st->field_type === 'decimal')
        type="number" step="0.0001" min="0" max="9999"
        @elseif($st->field_type === 'numeric')
        type="number" step="1" @if($st->key === 'FlsModule.ar_marginlrate') min="-9999" max="0" @else min="0" max="9999"
        @endif
        @else
