<div class="card mb-2">
  ...
  <div class="card-body p-0 table-responsive">
    <table class="table table-sm table-borderless table-striped text-nowrap mb-0">
      <tr>
        <th style="width: 30%;" scope="row">@lang('FlsModule::common.type')</th>
        <td>{{ $subfleet->type }}</td>
      </tr>
      @if($subfleet->fares_count > 0)
      <tr>
        <th scope="row">@lang('FlsModule::common.config')</th>
        <td>
      ...
      @endif
      <tr>
        <th scope="row">@lang('FlsModule::common.airline')</th>
        <td>
          <a href="{{ route('FlsModule.airline', [$subfleet->airline->icao ?? '']) }}">{{ $subfleet->airline->name ?? ''
            }}</a>
        </td>
      </tr>
      <tr>
        <th scope="row">@lang('FlsModule::common.base')</th>
        <td>
          <a href="{{ route('FlsModule.hub', [$subfleet->hub_id ?? '']) }}">{{ $subfleet->hub->full_name ?? '' }}</a>
        </td>
      </tr>
      @if($subfleet->flights_count > 0)
      <tr>
        <th scope="row">@lang('FlsModule::common.flights')</th>
