<table class="table table-sm table-borderless text-center text-nowrap align-middle mb-0">
  <thead>
    <tr>
      <th class="text-start">@sortablelink('flight_number', __('FlsModule::common.flightno'))</th>
      <th class="text-start">@sortablelink('dpt_airport_id', __('FlsModule::common.orig'))</th>
      <th class="text-start">@sortablelink('arr_airport_id', __('FlsModule::common.dest'))</th>
      @if(!isset($ac_page))
      <th>@sortablelink('aircraft.registration', __('FlsModule::common.aircraft'))</th>
      @endif
      <th>@sortablelink('flight_time', __('FlsModule::common.btime'))</th>
      <th>@sortablelink('fuel_used', __('FlsModule::common.fuelused'))</th>
      @ability('admin', 'admin-access')
      <th>@sortablelink('score', __('FlsModule::common.score'))</th>
      <th>@sortablelink('landing_rate', __('FlsModule::common.lrate'))</th>
      ...
      <th>Network</th>
      @endif
      <th class="text-end">@sortablelink('user.name', __('FlsModule::common.pilot'))</th>
      <th class="text-end">@sortablelink('submitted_at', __('FlsModule::common.submitted'))</th>
    </tr>
  </thead>
  <tbody>
    ...
    <tr @if ($pirep->state != 2) {!! Fls_PirepState($pirep, 'row') !!} @endif>
      ...
      <td>
        {{ Fls_ConvertWeight($pirep->fuel_used, $units['fuel']) }}
        @ability('admin', 'admin-access')
        @if(filled($pirep->simbrief) && ($pirep->fuel_used->local() - ($pirep->simbrief->xml->fuel->enroute_burn +
        ($pirep->simbrief->xml->fuel->contingency * 1.15) + ($pirep->simbrief->xml->fuel->taxi * 2)) > 100))
      ...
      @if(Theme::getSetting('gen_stable_approach'))
      <td>@widget('FlsModule::StableApproach', ['pirep' => $pirep])</td>
      @endif
      @endability
      @if(Fls_Setting('FlsModule.networkcheck', false))
