<table class="table table-sm table-borderless table-striped text-start text-nowrap mb-0 align-middle">
  <tr>
    ...
    <th class="text-center">@lang('FlsModule::common.state')</th>
    @endif
    @if(!isset($type))
  ...
  <tr @if(empty($state_badge) && $user->state != 1) {!! Fls_UserState($user, 'row') !!} @endif>
    <td>
    ...
    <td class="text-center">
      ...
      {{ Fls_ConvertMinutes(($user->flight_time + $user->transfer_time), '%2dh %2dm') }}
      @else
