...
<div class="accordion accordion-flush" id="StableApproachReport{{ $sap->id }}">
  ...
  <div class="accordion-item">
    <h5 class="accordion-header" id="stable-messages-heading{{ $sap->id }}">
      <button class="accordion-button p-1 collapsed" type="button" data-bs-toggle="collapse"
        data-bs-target="#stable-messages{{ $sap->id }}" aria-expanded="false"
        aria-controls="stable-messages{{ $sap->id }}">
        Analysis
      </button>
    </h5>
    <div id="stable-messages{{ $sap->id }}" class="accordion-collapse collapse"
      aria-labelledby="stable-messages-heading{{ $sap->id }}" data-bs-parent="#StableApproachReport{{ $sap->id }}">
      <div class="accordion-body p-0">
        <table class="table table-sm table-borderless table-striped mb-0 align-middle">
          ...
          <tr>
            ...
            <td>
              <i @if($analysis->aircraft_profile_type->va === true)
          ...
          <tr>
            ...
            <td>
              {{ Form::open(['route' => 'FlsModule.stable_update', 'method' => 'post']) }}
              {{ Form::hidden('report_id', $sap->id) }}
              {{ Form::hidden('current_page', url()->current()) }}
              {{ Form::button('Approve as Stable', ['type' => 'submit', 'name' => 'operation', 'value' => 'update',
              'class' => 'btn btn-success btn-sm m-0 mx-1 p-0 px-1', 'onclick' => "return confirm('Do you really want to
              accept this report as STABLE ?')"]) }}
              {{ Form::button('Delete', ['type' => 'submit', 'name' => 'operation', 'value' => 'delete', 'class' => 'btn
              btn-danger btn-sm m-0 mx-1 p-0 px-1', 'onclick' => "return confirm('Do you really want to DELETE this
              report ?')"]) }}
              {{ Form::close() }}
            </td>
          </tr>
  ...
  <div class="accordion-item">
    ...
    <h5 class="accordion-header" id="stable-airport-heading{{ $sap->id }}">
      <button class="accordion-button p-1 collapsed" type="button" data-bs-toggle="collapse"
        data-bs-target="#stable-airport{{ $sap->id }}" aria-expanded="false"
        aria-controls="stable-airport{{ $sap->id }}">
        Airport & Runway Details
      </button>
    </h5>
    <div id="stable-airport{{ $sap->id }}" class="accordion-collapse collapse"
      aria-labelledby="stable-airport-heading{{ $sap->id }}" data-bs-parent="#StableApproachReport{{ $sap->id }}">
      <div class="accordion-body p-0">
        <table class="table table-sm table-borderless table-striped mb-0 align-middle">
          <tr>
  ...
  <div class="accordion-item">
    ...
    <h5 class="accordion-header" id="stable-approach-heading{{ $sap->id }}">
      <button class="accordion-button p-1 collapsed" type="button" data-bs-toggle="collapse"
        data-bs-target="#stable-approach{{ $sap->id }}" aria-expanded="false"
        aria-controls="stable-approach{{ $sap->id }}">
        Approach
      </button>
    </h5>
    <div id="stable-approach{{ $sap->id }}" class="accordion-collapse collapse"
      aria-labelledby="stable-approach-heading{{ $sap->id }}" data-bs-parent="#StableApproachReport{{ $sap->id }}">
      <div class="accordion-body p-0">
        <table class="table table-sm table-borderless table-striped mb-0 align-middle">
          <tr>
          ...
          @if($approach->loc_dev->max < 9) <tr>
            ...
            @if($approach->gs_dev->max < 9) <tr>
              ...
              <tr>
                ...
                <td>{{ number_format($approach->sinkrate->min, 2).' fpm | '.number_format($approach->sinkrate->max, 2).'
                  fpm' }}</td>
              </tr>
              <tr>
                <th>Indicated Air Speed (Min/Max)</th>
                <td>{{ number_format($approach->kias->min, 2).' kt | '.number_format($approach->kias->max, 2).' kt' }}
                </td>
              </tr>
              <tr>
  ...
  <div class="accordion-item">
    <h5 class="accordion-header" id="stable-touchdowns-heading{{ $sap->id }}">
      <button class="accordion-button p-1 collapsed" type="button" data-bs-toggle="collapse"
        data-bs-target="#stable-touchdowns{{ $sap->id }}" aria-expanded="false"
        aria-controls="stable-touchdowns{{ $sap->id }}">
        @if(count($sap->touchdowns) > 1) Touchdowns @else Touchdown @endif
      </button>
    </h5>
    <div id="stable-touchdowns{{ $sap->id }}" class="accordion-collapse collapse"
      aria-labelledby="stable-touchdowns-heading{{ $sap->id }}" data-bs-parent="#StableApproachReport{{ $sap->id }}">
      <div class="accordion-body p-0">
  ...
  <div class="accordion-item">
    <h5 class="accordion-header" id="stable-touchdown-heading{{ $sap->id }}">
      <button class="accordion-button p-1 collapsed" type="button" data-bs-toggle="collapse"
        data-bs-target="#stable-touchdown{{ $sap->id }}" aria-expanded="false"
        aria-controls="stable-touchdown{{ $sap->id }}">
        Touchdowns Combined Analysis
      </button>
    </h5>
    <div id="stable-touchdown{{ $sap->id }}" class="accordion-collapse collapse"
      aria-labelledby="stable-touchdown-heading{{ $sap->id }}" data-bs-parent="#StableApproachReport{{ $sap->id }}">
      <div class="accordion-body p-0">
        <table class="table table-sm table-borderless table-striped mb-0 align-middle">
          <tr>
            <th>Threshold Distance (Min/Max)</th>
            <td>{{ number_format($touchdown_combined->threshold_dist->min, 2).' m |
              '.number_format($touchdown_combined->threshold_dist->max, 2).' m' }}</td>
          </tr>
          <tr>
            <th>Pitch (Min/Max)</th>
            <td>{{ number_format($touchdown_combined->pitch->min, 2).' deg |
              '.number_format($touchdown_combined->pitch->max, 2).' deg' }}</td>
          </tr>
          <tr>
            <th>Max Roll & Yaw</th>
            <td>{{ number_format($touchdown_combined->bank->max, 2).' deg |
              '.number_format($touchdown_combined->crab->max, 2).' deg' }}</td>
          </tr>
          <tr>
            <th>Max Rate & G-Force</th>
            <td>{{ number_format($touchdown_combined->fpm_agl->max, 2).' fpm |
              '.number_format($touchdown_combined->g_vertical->max, 2).' g' }}</td>
          </tr>
          <tr>
            <th>Centerline Deviation</th>
  ...
  <div class="accordion-item">
    ...
    <h5 class="accordion-header" id="stable-rollout-heading{{ $sap->id }}">
      <button class="accordion-button p-1 collapsed" type="button" data-bs-toggle="collapse"
        data-bs-target="#stable-rollout{{ $sap->id }}" aria-expanded="false"
        aria-controls="stable-rollout{{ $sap->id }}">
        Rollout
      </button>
    </h5>
    <div id="stable-rollout{{ $sap->id }}" class="accordion-collapse collapse"
      aria-labelledby="stable-rollout-heading{{ $sap->id }}" data-bs-parent="#StableApproachReport{{ $sap->id }}">
      <div class="accordion-body p-0">
        <table class="table table-sm table-borderless table-striped mb-0 align-middle">
          <tr>
