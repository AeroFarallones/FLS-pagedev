  ...
  <div class="card-body p-0 overflow-auto table-responsive">
    <table class="table table-borderless table-sm table-striped text-start text-nowrap align-middle mb-0">
      <tr>
        ...
        <th>ETD</th>
        @endif
        <th class="text-end">@lang('FlsModule::common.expire')</th>
      </tr>
      ...
      <tr>
        @if(!$bids)
        <td>
          <a href="{{ route('FlsModule.aircraft', [$booking->aircraft->registration]) }}"
            title="{{ optional($booking->aircraft)->name }}">{{ optional($booking->aircraft)->ident }}</a>
        </td>
        @endif
        <td>
          <a href="{{ route('frontend.flights.show', [$booking->flight_id]) }}" title="{{ $booking->flight->ident }}">{{
            optional($booking->flight->airline)->code.' '.$booking->flight->flight_number }}</a>
        </td>
        <td>
          <a href="{{ route('frontend.airports.show', [$booking->flight->dpt_airport_id]) }}"
            title="{{ optional($booking->flight->dpt_airport)->name }}">{{ $booking->flight->dpt_airport_id }}</a>
        </td>
        <td>
          <a href="{{ route('frontend.airports.show', [$booking->flight->arr_airport_id]) }}"
            title="{{ optional($booking->flight->arr_airport)->name }}">{{ $booking->flight->arr_airport_id }}</a>
        </td>
        <td>
