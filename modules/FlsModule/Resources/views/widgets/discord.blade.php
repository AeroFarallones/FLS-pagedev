...
<div class="card mb-2">
  ...
  <div class="card-footer p-1 text-end small">
    <span class="float-start pt-1">
      {{ count($members) }} @lang('FlsModule::widgets.donline')
      @if(count($channels) > 0)
      {{ ' | '.count($channels) }} @lang('FlsModule::widgets.dchannels')
      @endif
    </span>
    @if(filled($invite))
    <a class="btn btn-sm bg-success text-black p-0 px-2" href="{{ $invite }}"
      target="_blank">@lang('FlsModule::widgets.djoin')</a>
    @endif
  </div>
</div>
@endif