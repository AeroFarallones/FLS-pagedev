...
<div class="card mb-2">
  <div class="card-header p-1">
    <h5 class="m-1">
      @lang('FlsModule::widgets.fleetoverview')
  ...
  <div class="card-body p-0 overflow-auto table-responsive">
    <table class="table table-sm table-borderless table-striped text-start text-nowrap align-middle mb-0">
      <tr>
        <th>{{ $col_header }}</th>
        <th class="text-end pe-1">@lang('FlsModule::common.count')</th>
      </tr>
      @foreach ($fleet as $ac)
      <tr>
        <td>
          ...
          <a href="{{ route('FlsModule.subfleet', [$ac->subfleet->type ?? '']) }}">{{ optional($ac->subfleet)->name.' |
            '.optional(optional($ac->subfleet)->airline)->icao }}</a>
          @else
          <a href="{{route('frontend.airports.show', [$ac->airport_id])}}">{{ $ac->airport->name ?? $ac->airport_id
            }}</a>
