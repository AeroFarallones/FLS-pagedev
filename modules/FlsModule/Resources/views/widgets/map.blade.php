@if(isset($flights) && $flights > 0 || !isset($flights) && count($mapAirports) > 0 || !isset($flights) &&
count($mapHubs) > 0)
{{-- Map Modal Button --}}
<div class="row">
  <div class="col d-grid">
    <button type="button" class="btn btn-sm btn-danger p-0 px-1" data-toggle="modal"
      ...
      @lang('FlsModule::widgets.fleet_map')
      @elseif($mapsource === 'airline')
      @lang('FlsModule::widgets.airline_map')
      @elseif($mapsource === 'assignment')
      @lang('FlsModule::widgets.assignm_map')
      @elseif($mapsource === 'aerodromes')
      @lang('FlsModule::widgets.aerodr_map')
      @else
      @lang('FlsModule::widgets.flights_map')
      @endif
    </button>
  </div>
</div>
...
{{-- Map Modal --}}
<div class="modal" id="modal{{ $mapsource }}" data-backdrop="static" data-keyboard="false" tabindex="-1"
  aria-labelledby="{{ $mapsource.'Title' }}" aria-hidden="true">
    ...
    <div class="modal-content shadow-none p-0">
      ...
      <div class="modal-footer border-0 p-0 small text-end">
        <span>
          ...
          @lang('FlsModule::widgets.citypairs'): {{ count($mapCityPairs) }} |
          @endif
          @lang('FlsModule::widgets.hubs'): {{ count($mapHubs) }} |
          @lang('FlsModule::widgets.airports'): {{ count($mapAirports) }}
          @if(isset($flights))
