    ...
    <table class="table table-borderless table-sm table-striped text-center text-nowrap mb-0">
      ...
      <tr>
        <td class="text-start">
          ...
          <a href="{{ route('frontend.profile.show', [$pilot['user_id']]) }}" target="_blank">{{ $pilot['name_private']
            }}</a>
          @else
          Deleted User
          @endif
        ...
        <td class="text-end">
          ...
          @elseif($pilot['pirep'])
          <i class="fas fa-clipboard-check text-success mx-1"
            title="{{ $pilot['pirep']->aircraft->icao.' | '.$pilot['pirep']->dpt_airport_id.' > '.$pilot['pirep']->arr_airport_id }}"></i>
          @endif
          @endif
          {{ Fls_ConvertMinutes($pilot['online_time']) }}
        </td>
      </tr>
      @endforeach
    </table>
