...
class Fls_FleetServices
{
    ...
    public function ParkAircraft($reg)
    {
        ...
        if ($aircraft) {
            ...
            if ($pirep) {
                ...
                Log::info('Fls Basic, Pirep ID:' . $pirep->id . ' CANCELLED');
            }

            $aircraft->state = AircraftState::PARKED;
            $aircraft->save();
            $result = $result + 1;
            Log::info('Fls Basic, Aircraft REG:' . $aircraft->registration . ' PARKED');
        }
    ...
    public function AverageFuelBurn($aircraft_id)
    {
        ...
        $result = Fls_Tech::where('icao', $aircraft_icao)->value('avg_fuel');
