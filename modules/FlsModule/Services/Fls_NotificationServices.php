...
class Fls_NotificationServices
{
    public function PirepMessage($pirep, $content)
    {
        $wh_url = Fls_Setting('FlsModule.discord_pirep_webhook');
        $message_poster = !empty(Fls_Setting('FlsModule.discord_pirep_msgposter')) ? Fls_Setting('FlsModule.discord_pirep_msgposter') : config('app.name');
                ...
                [
                    ...
                    'fields' => [
                        [
                            ...
                            'name'   => '__Block Time__',
                            'value'  => Fls_ConvertMinutes($pirep->flight_time),
                            'inline' => true
                        ], [
                            'name'   => '__Equipment__',
    ...
    public function DiscordNotification($webhook_url, $json_data)
    {
        ...
        if ($response) {
            Log::debug('Fls Basic | Discord WebHook Msg Response: ' . $response);
        }
        curl_close($ch);
    }
}
