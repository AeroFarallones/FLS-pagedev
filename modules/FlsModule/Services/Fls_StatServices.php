...
class Fls_StatServices
{
    ...
    public function PersonalStats($user_id, $period, $type)
    {
        ...
        if ($period === 'currenty' || $period === 'lasty' || $period === 'prevy') { // Years
            ...
            $personal['period_text'] = __('FlsModule::dates.' . $b_date->format('m'));
        } elseif (is_numeric($period)) { // Days
            $s_date = $b_date->copy()->startOfDay()->subdays($period);
            $e_date = $now->endOfDay();
            $personal['period_text'] = __('FlsModule::widgets.lastndays', ['period' => $period]);
        ...
        if ($type === 'avglanding') {
            ...
            $personal['formatted'] = Fls_ConvertMinutes(round($result), '%2dh %2dm');
    ...
    public function BasicStats($airline_id = null)
    {
        ...
        if (empty($airline_id)) {
            $stats[__('FlsModule::common.airlines')] = Airline::where('active', 1)->count();
        }

        $stats[__('FlsModule::common.pilots')] = User::where($where)->count();
        $stats[__('FlsModule::common.subfleets')] = count($subfleets_array);
        $stats[__('FlsModule::common.aircraft')] = Aircraft::whereIn('subfleet_id', $subfleets_array)->count();
        $stats[__('FlsModule::common.flights')] = Flight::where($where)->count();
    ...
    public function PirepStats($airline_id = null, $aircraft_id = null)
    {
        ...
        $stats[__('FlsModule::widgets.pireps_ack')] = Pirep::where($where)->count();

        // Return empty array if pirep count is zero, no need to work for the rest
        if ($stats[__('FlsModule::widgets.pireps_ack')] === 0) {
            return array();
        }
        ...
        if (setting('pireps.delete_rejected_hours') == 0 && $level > 10) {
            if (empty($airline_id)) {
                $stats[__('FlsModule::widgets.pireps_rej')] = Pirep::where('state', PirepState::REJECTED)->count();
            } else {
                $stats[__('FlsModule::widgets.pireps_rej')] = Pirep::where(['airline_id' => $airline_id, 'state' => PirepState::REJECTED])->count();
            }
        }
        */
        ...
        if ($pax_amount > 0) {
            $stats[__('FlsModule::widgets.pireps_pax')] = number_format($pax_amount);
            $stats[__('FlsModule::widgets.avg_pax')] = number_format($pax_avg);
        }

        if ($cgo_amount > 0) {
            $stats[__('FlsModule::widgets.pireps_cgo')] = number_format($cgo_amount) . ' ' . $unit_weight;
            $stats[__('FlsModule::widgets.avg_cgo')] = number_format($cgo_avg) . ' ' . $unit_weight;
        }
        ...
        $stats[__('FlsModule::widgets.ttime')] = Fls_ConvertMinutes($total_time, '%2dh %2dm');
        if ($level > 10) {
            $stats[__('FlsModule::widgets.atime')] = Fls_ConvertMinutes($average_time, '%2dh %2dm');
        }

        $stats[__('FlsModule::widgets.tfuel')] = number_format($total_fuel) . ' ' . $unit_fuel;
        if ($level > 10) {
            $stats[__('FlsModule::widgets.afuel')] = number_format($average_fuel) . ' ' . $unit_fuel;
        }

        if ($total_fuel > 0 && $total_time > 0) {
            $average_fuel_hour = ($total_fuel / $total_time) * 60;
            $stats[__('FlsModule::widgets.hfuel')] = number_format($average_fuel_hour) . ' ' . $unit_fuel;
        }

        $stats[__('FlsModule::widgets.tdist')] = number_format($total_dist) . ' ' . $unit_distance;
        if ($level > 10) {
            $stats[__('FlsModule::widgets.adist')] = number_format($average_dist) . ' ' . $unit_distance;
        }

        if ($total_dist > 0 && $total_time > 0 && $level > 10) {
            $average_dist_hour = ($total_dist / $total_time) * 60;
            $stats[__('FlsModule::widgets.hdist')] = number_format($average_dist_hour) . ' ' . $unit_distance;
        }

        $where['source'] = PirepSource::ACARS;

        $average_lrate = Pirep::where($where)->avg('landing_rate');
        $stats[__('FlsModule::widgets.alrate')] = number_format(abs($average_lrate)) . ' ft/min';

        if ($level > 10) {
            $average_score = Pirep::where($where)->avg('score');
            $stats[__('FlsModule::widgets.ascore')] = number_format($average_score);
        }
    ...
    public function AirlineFinance($journal_id)
    {
        ...
        $finance[__('FlsModule::common.income')] = money($income, $currency);
        $finance[__('FlsModule::common.expense')] = money($expense, $currency);
        $finance[__('FlsModule::common.balance')] = '<span style="color: ' . $color . ';"><b>' . money($balance, $currency) . '</b></span>';
