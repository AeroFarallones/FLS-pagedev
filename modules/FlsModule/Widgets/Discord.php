...
use Modules\FlsModule\Models\Fls_Discord;
use Theme;
...
class Discord extends Widget
{
    ...
    public function run()
    {
        ...
        $discord = Fls_Discord::where('server_id', $server_id)->first();
        ...
        if (!$discord || $discord->updated_at->diffInSeconds() > $this->reloadTimeout) {
            ...
            try {
                ...
                if ($response->getStatusCode() == 200) {
                    ...
                    Log::error('Fls Basic | HTTP ' . $response->getStatusCode() . ' error occured during download !');
                }
            } catch (GuzzleException $e) {
                Log::error('Fls Basic | Discord Widget Download Error : ' . $e->getMessage());
            }

            $discord = Fls_Discord::updateOrCreate(['server_id' => $server_id], $model_data);
        }
        ...
        return view('FlsModule::widgets.discord', [
            'channels'   => $channels,
