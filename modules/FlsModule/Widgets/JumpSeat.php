...
class JumpSeat extends Widget
{
    ...
    public function run()
    {
        ...
        $form_route = 'FlsModule.jumpseat';
        $icon_color = 'danger';
        $icon_title = __('FlsModule::widgets.js_title_auto');

        if ($price === 'free') {
            $icon_color = 'success';
            $icon_title = __('FlsModule::widgets.js_title_free');
        } elseif (is_numeric($price)) {
            $icon_color = 'primary';
            $icon_title = __('FlsModule::widgets.js_title_fixed') . ' ' . number_format($price) . ' ' . setting('units.currency');
        }

        return view('FlsModule::widgets.jumpseat_travel', [
