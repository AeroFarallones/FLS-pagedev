...
class Map extends Widget
{
    ...
    public function run()
    {
        ...
        elseif ($type === 'assignment') {
            ...
            // Needs Fls Special Module
            if (Fls_CheckModule('FlsSpecial')) {
                $now = Carbon::now();
                $asg_where = [];
                $asg_where['assignment_year'] = $now->year;
                $asg_where['assignment_month'] = $now->month;
                $asg_where['user_id'] = $user->id;
                $latest_assignments = \Modules\FlsSpecial\Models\DS_Assignment::where($asg_where)->pluck('flight_id')->toArray();
            }
        ...
        if ($type != 'fleet' && $type != 'aerodromes') {
            ...
            foreach ($mapflights as $mf) {
                ...
                if (Fls_InArray_MD($reverse, $citypairs)) {
                    continue;
                }
            ...
            $citypairs = Fls_ArrayUnique_MD($citypairs, 'name');
        ...
        // Routes For PopUps
        $hroute = 'FlsModule.hub';
        $aroute = 'FlsModule.aircraft';
        ...
        return view('FlsModule::widgets.map', [
            'mapcenter'    => $mapcenter,
            'mapsource'    => $type,
