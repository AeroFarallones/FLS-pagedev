...
use Modules\FlsModule\Models\Fls_RandomFlight;
use Modules\FlsModule\Services\Fls_FlightServices;
...
class RandomFlights extends Widget
{
    ...
    public function run()
    {
        ...
        if (!$user) {
            ...
            return view('FlsModule::widgets.random_flights', [
                'is_visible' => false,
            ]);
        }

        Fls_RandomFlight::where('assign_date', '!=', $today)->delete();
        ...
        $rfs = Fls_RandomFlight::with($eager_load)->where($whereRF)->get();

        if ($rfs->isEmpty()) {
            $FlightSvc = app(Fls_FlightServices::class);
