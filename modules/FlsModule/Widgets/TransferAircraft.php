...
class TransferAircraft extends Widget
{
    ...
    public function run()
    {
        ...
        $form_route = 'FlsModule.transferac';
        $icon_color = 'danger';
        $icon_title = __('FlsModule::widgets.ta_title_auto');

        if ($price === 'free') {
            $icon_color = 'success';
            $icon_title = __('FlsModule::widgets.ta_title_free');
        } elseif (is_numeric($price)) {
            $icon_color = 'primary';
            $icon_title = __('FlsModule::widgets.ta_title_fixed') . ' ' . number_format($price) . ' ' . setting('units.currency');
        }
        ...
        return view('FlsModule::widgets.transfer_aircraft', [
