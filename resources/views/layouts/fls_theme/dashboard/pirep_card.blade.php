<div class="card dashboard-pirep-card dashboard-card pt-4 h-100">
    ...
    <div class="row dashboard-row-pirep dashboard-secondary-text">
        <div class="col-sm-6">
            <a class="text-white a-fls"
                href="{{ route('frontend.pireps.show', [$pirep->id]) }}">{{ ($pirep->aircraft)->ident }}</a>
        </div>
        <div class="col-sm-6">
    ...
    <div class="row dashboard-row-pirep">
        ...
        <i class="fa-solid fa-circle text-warning text-center dashboard-icon-size" title="Accepted"
            aria-hidden="true"></i>
        @elseif($pirep->state === PirepState::ACCEPTED)
        <i class="fa-solid fa-circle text-success text-center dashboard-icon-size" title="Accepted"
            aria-hidden="true"></i>
        @elseif($pirep->state === PirepState::REJECTED)
        <i class="fa-solid fa-circle text-danger text-center dashboard-icon-size" title="Accepted"
            aria-hidden="true"></i>
        @endif
    </div>
    <div class="row dashboard-row-pirep dashboard-secondary-text">
