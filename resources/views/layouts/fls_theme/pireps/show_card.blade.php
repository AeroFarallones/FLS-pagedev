<div class="card mb-2" style="border-color: #00157f;">
    ...
    <div class="card-footer bg-transparent p-1" style="border-color: #00157f;">
        <div class="progress" style="height: 20px;">
            <div class="progress-bar-fls @if(blank($pirep->block_on_time)) progress-bar-striped progress-bar-animated @endif text-white db-font-montbold text-center"
        ...
        <div class="row">
            ...
            <div class="col-2 text-end db-font-montbold">
                <form method="post" action="{{ route('frontend.pireps.submit', $pirep->id) }}">
                    @csrf
                    <button
                        class="btn btn-sm button-blue-fls text-white m-0 mx-1 p-0 px-1 float-end">@lang('common.submit')</button>
                </form>
                <form method="get" action="{{ route('frontend.pireps.edit', $pirep->id) }}">
                    @csrf
                    <button
                        class="btn btn-sm button-blue-fls text-white m-0 mx-1 p-0 px-1 float-end">@lang('common.edit')</button>
                </form>
            </div>
            @endif
        </div>
