...
if ($dep_cost < ($arr_cost * $factor)) { return $result_ok; } return $result_no; } } // Check URL // Return boolean if
  (!function_exists('DT_CheckUrl')) { function DT_CheckUrl($url=null) { if (!$url) { return false; }
  $headers=get_headers($url); if ($headers && strpos($headers[0], '200' )) { return true; } else { return false; } } }
  // Get image files from a directory // Return array or null if (!function_exists('DT_GetImages')) { function
  DT_GetImages($dir=null) { $dir=isset($dir) ? $dir : 'image/carousel/' ; $type=isset($type) ? $type : 'bs5' ; if
  (is_dir($dir)) { if ($dh=opendir($dir)) { while (($file=readdir($dh)) !==false) { if (stripos($file, '.jpg' ) !==false
  || stripos($file, '.jpeg' ) !==false || stripos($file, '.png' ) !==false) { $image_files[]=$dir.$file; } }
  closedir($dh); } } if (isset($image_files)) { shuffle($image_files); } return isset($image_files) ? $image_files :
  null; } } // Get Required Units // Return array if (!function_exists('DT_GetUnits')) { function
  DT_GetUnits($type=null) { $units=[]; $units['currency']=setting('units.currency');
  $units['distance']=setting('units.distance'); $units['fuel']=setting('units.fuel');
  $units['weight']=setting('units.weight'); if ($type==='full' ) { $units['volume']=settings('units.volume');
  $units['altitude']=settings('units.altitude'); } return $units; } } // Convert Distance / Nautical Miles // Return
  string if (!function_exists('DT_ConvertDistance')) { function DT_ConvertDistance($distance, $unit=null) {
  $unit=isset($unit) ? $unit : setting('units.distance'); if (!$distance[$unit]> 1) {
  return null;
  }
  ...
  if ($minutes < 1) { return null; } $hours=floor($minutes / 60); $minutes=($minutes % 60); return sprintf($format,
    $hours, $minutes); } } // Convert Weight / Pounds // Return string if (!function_exists('DT_ConvertWeight')) {
    function DT_ConvertWeight($value, $target_unit=null) { $target_unit=isset($target_unit) ? $target_unit :
    setting('units.weight'); if (!$value[$target_unit]> 0) {
    return null;
    }
    ...
    for ($i=0; $i<7; $i++) { if ($flight_days & pow(2, $i)) { $days[]=jddayofweek($i, 1); } } return implode(', ', $days);;
    }
  }

  // Decode Flight Type
  // Return mixed (plain text or bootstrap badge/button)
  if (!function_exists(' DT_FlightType')) { function DT_FlightType($flight_type, $type=null) {
      $flight_type=FlightType::label($flight_type); if ($type==='badge' ) {
      ...
      $route_leg = '<span class="btn btn-sm bg-warning m-0 mx-1 p-0 px-1 disabled text-black"> Leg#
        '.$route_leg.'</span>';
      }
      ...
      if ($value > 0) {
      $error = ' <i class="fas fa-exclamation-triangle mx-2" style="color:firebrick;"
        title="Positive Landing Rate !"></i>';
      }
      ...
      if ($value < 0) {
        ...
        if ($value <=10) { $value=number_format($value, 2).' '.$units[' fuel'].$error; } else {
        $value=number_format($value).' '.$units[' fuel'].$error; } } // Weight Values elseif
        (str_contains($slug, '-weight' )) { if ($units['weight']==='kg' ) { $value=$value / 2.20462262185; }
        $value=number_format($value).' '.$units[' weight'].$error; } // Pitch, Roll, Heading : Angle elseif
        (str_contains($slug, 'roll' ) || str_contains($slug, 'pitch' ) || str_contains($slug, 'heading' )) { //
        $value=number_format($value,2)."&deg;".$error; $value=$value.'&deg;'.$error; } // Centerline Deviation :
        Distance elseif (str_contains($slug, 'centerline-dev' )) { if ($units['distance']==='km' ) {
        $value=number_format(($value / 3.2808), 2).' m'.$error; } else { $value=number_format($value, 2).' ft'.$error; }
        } // TakeOff and Landing Speeds elseif (str_contains($slug, '-speed' )) { $value=number_format($value).' kts'; }
        } // Date/Time Values (not displaying full date on purpose) elseif (str_contains($slug, 'time-real' ) ||
        str_contains($slug, 'time-sim' )) { $value=Carbon::parse($value)->format('H:i').' UTC';
        }
        ...
        // User Details
        $vasys['PersonId'] = optional($pirep->user->fields->firstWhere('name',
        Theme::getSetting('gen_ivao_field')))->value;
        ...
        $vasys['TasCruise'] = '410'; // Not possible to calculate it without indicated/calibrated airspeed and ISA
        Deviation!
        ...
        $vasys['AltAirport'] = filled(optional($pirep->flight)->alt_airport_id) ? $pirep->flight->alt_airport_id :
        'NONE';
